...
1. [toolkit](./toolkit/README.md): ecs 架构的精简游戏引擎框架，是其他项目的基础，需要指出这是一个游戏引擎框架而不是单纯的渲染框架。这里面也包含一个 editor 能够对场景提供可视化编辑，可以增改一个实体（entity）对应的组件（component），调整各个系统的设置并序列化到文件。场景和 bundle 的序列化是通过静态反射实现的；在这个框架中增加组件，材质，插件步骤非常简单，框架中本身也有很多例子。

...
source_dir = r'D:\Code\ai\motion_processing\arfriend_data\source'
output_dir = r'D:\Code\ai\motion_processing\arfriend_data\retarget'
target_skel_filepath = r'D:\Code\ai\motion_processing\arfriend_data\skel.bvh'
...
for source_file in source_files:
    if not source_file.endswith(".bvh"):
        continue
    source_target_file_pair.append((os.path.join(source_dir, source_file), target_skel_filepath))

# retarget all source motions to all target skeletons
for source_filepath, target_filepath in source_target_file_pair:
    app.FileNew()
    # configure the target biped character
    app.FileImport(target_filepath, False)
    null_node = FBFindModelByLabelName("BVH:reference")
    if null_node:
        null_node.FBDelete()
    null_node = FBFindModelByLabelName("BVH:root")
    if null_node:
        null_node.FBDelete()
    character = characterizeBiped("BVH:", mobuMap_target)

    deselectAll()
    newTake = FBTake(os.path.basename(source_filepath))
    scene.Takes.append(newTake)
    setCurrentTake(os.path.basename(source_filepath))
    newTake.ClearAllProperties(False)
    app.FileImport(source_filepath, False)
    # set frame rate
    FBPlayerControl().SetTransportFps(FBTimeMode.kFBTimeMode30Frames)
    motionCharacter = characterizeBiped(f"BVH 1:", mobuMap_source)
    ...
    exportPath = os.path.join(output_dir, os.path.basename(source_filepath))
    print(f"Retarget motion {source_filepath} to {exportPath}")

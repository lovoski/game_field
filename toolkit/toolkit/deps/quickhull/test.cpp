...
#define VERIFY(x)                                                              \
  if (!(x)) {                                                                  \
    std::cout << "test failed: '" << #x << "' is not true." << std::endl;      \
  }
...
template <typename T>
static std::vector<Vector3<T>>
createSphere(T radius, size_t M, Vector3<T> offset = Vector3<T>(0, 0, 0)) {
...
static void testHalfEdgeOutput() {
  ...
  HalfEdgeMesh<FloatType, size_t> mesh =
      qh.getConvexHullAsMesh(&pc[0].x, pc.size(), true);
...
int run() {
  ...
  assert(hull.getIndexBuffer().size() ==
         3 * 2 * 6); // 6 faces, 2 triangles/face, 3 indices/tri
...
int main() { return quickhull::tests::run(); }

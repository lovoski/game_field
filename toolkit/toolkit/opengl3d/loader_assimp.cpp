  ...
  for (auto &p : node_mapping) {
    for (int i = 0; i < p.first->mNumMeshes; i++) {
      ...
      for (int j = 0; j < mesh->mNumBones; j++) {
        ...
        auto bone_node_comp =
            registry.try_get<opengl3d::bone_node>(bone_entity);
        ...
        if (bone_node_comp == nullptr) {
          ...
          registry.emplace<opengl3d::bone_node>(bone_entity);
          bone_node_comp = registry.try_get<opengl3d::bone_node>(bone_entity);
        }
...
void process_mesh(entt::registry &registry, entt::entity container,
  ...
  auto &mesh_data = registry.emplace<opengl3d::mesh_data>(container);
  mesh_data.mesh_name =
  ...
  if (mesh->mNumBones > 0) {
    auto &bundle_data = registry.get<opengl3d::skinned_mesh_bundle>(
  ...
  opengl3d::init_opengl_buffers(mesh_data);
}

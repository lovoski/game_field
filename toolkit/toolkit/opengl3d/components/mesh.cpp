...
void mesh_data::draw_gui(entt::registry &registry, entt::entity entity) {
  ...
  if (ImGui::TreeNode("Material Settings")) {
    ...
    texture_select(material.albedo_tex, material.albedo_tex_filepath,
                   dynamic_cast<engine3d *>(registry.ctx().get<iapp *>())
                       ->checkerboard_tex);
    ImGui::SeparatorText("Normal Texture");
    texture_select(material.normal_tex, material.normal_tex_filepath,
                   dynamic_cast<engine3d *>(registry.ctx().get<iapp *>())
                       ->checkerboard_tex);

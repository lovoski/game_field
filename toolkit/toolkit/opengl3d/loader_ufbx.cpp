...
read_nodes(entt::registry &registry, ufbx_scene *scene, std::string filename,
  ...
  for (int i = 0; i < scene->nodes.count; i++) {
    ...
    if (scene->nodes[i]->bone) {
      auto &bone_comp = registry.emplace<opengl3d::bone_node>(
...
void read_mesh(entt::registry &registry, ufbx_mesh *mesh,
  ...
  if (skinned_mesh) {
    ...
    auto &bundle_comp =
        registry.get<opengl3d::skinned_mesh_bundle>(bundle_entity);
  ...
  for (int pi = 0; pi < mesh->material_parts.count; pi++) {
    ...
    auto &mesh_comp = registry.emplace<opengl3d::mesh_data>(mesh_entity);
    ...
    if (skinned_mesh) {
      auto &bundle_comp = registry.get<opengl3d::skinned_mesh_bundle>(
    ...
    opengl3d::init_opengl_buffers(mesh_comp);
  }
}
        ...
        for (int j = 0; j < deformer->clusters.count; j++) {
          ...
          for (int k = 0; k < bone_entities.size(); k++) {
            if (bone_entities[k] == bone_entity) {
              auto &bone_node = registry.get<opengl3d::bone_node>(bone_entity);

...
static void sceneEmpty(Solver *solver) { solver->clear(); }
...
static void sceneDynamicFriction(Solver *solver) {
  ...
  new Rigid(solver, {100, 1}, 0.0f, 0.5f, {0, 0, 0}, {0, 0, 0});
  for (int x = 0; x <= 10; x++)
    new Rigid(solver, {1, 0.5f}, 1.0f, 5.0f - (x / 10.0f * 5.0f),
              {-30.0f + x * 2.0f, 0.75f, 0}, {10.0f, 0, 0});
}
...
static void sceneStaticFriction(Solver *solver) {
  ...
  for (int y = 0; y <= 10; y++)
    new Rigid(solver, {5, 0.5f}, 1.0f, 1.0f,
              {0.0f, y * 1.0f + 1.0f, 3.14159f / 6.0f});
}
...
static void scenePyramid(Solver *solver) {
  ...
  for (int y = 0; y < SIZE; y++)
    for (int x = 0; x < SIZE - y; x++)
      new Rigid(solver, {1, 0.5f}, 1.0f, 0.5f,
                {x * 1.1f + y * 0.5f - SIZE / 2.0f, y * 0.85f, 0.0f});
}
...
static void sceneRope(Solver *solver) {
  ...
  Rigid *prev = 0;
  for (int i = 0; i < 20; i++) {
    Rigid *curr = new Rigid(solver, {1, 0.5f}, i == 0 ? 0.0f : 1.0f, 0.5f,
                            {(float)i, 10.0f, 0.0f});
    if (prev)
      new Joint(solver, prev, curr, {0.5f, 0}, {-0.5f, 0},
...
static void sceneHeavyRope(Solver *solver) {
  ...
  for (int i = 0; i < N; i++) {
    Rigid *curr = new Rigid(
        solver, i == N - 1 ? math::vector2{SIZE, SIZE} : math::vector2{1, 0.5f},
        i == 0 ? 0.0f : 1.0f, 0.5f,
        {(float)i + (i == N - 1 ? SIZE / 2 : 0), 10.0f, 0.0f});
    if (prev)
      new Joint(solver, prev, curr, {0.5f, 0},
                i == N - 1 ? math::vector2{-SIZE / 2, 0}
                           : math::vector2{-0.5f, 0},
                {INFINITY, INFINITY, 0.0f});
    prev = curr;
  }
...
static void sceneHangingRope(Solver *solver) {
  ...
  for (int i = 0; i < N; i++) {
    Rigid *curr = new Rigid(
        solver,
        i == N - 1 ? math::vector2{SIZE, SIZE} : math::vector2{0.5f, 1.0f},
        i == 0 ? 0.0f : 1.0f, 0.5f,
        {0.0f, 10.0f - ((float)i + (i == N - 1 ? SIZE / 2 : 0)), 0.0f});
    if (prev)
      new Joint(solver, prev, curr, {0, -0.5f},
                i == N - 1 ? math::vector2{0, SIZE / 2}
                           : math::vector2{0, 0.5f},
                {INFINITY, INFINITY, 0.0f});
    prev = curr;
  }
...
static void sceneSpringsRatio(Solver *solver) {
  ...
  for (int i = 0; i < N; i++) {
    Rigid *curr =
        new Rigid(solver, {1, 0.5f}, i == 0 || i == N - 1 ? 0.0f : 1.0f, 0.5f,
                  {(float)i * 4, 10.0f, 0.0f});
    if (prev)
      new Spring(solver, prev, curr, {0.5f, 0}, {-0.5f, 0},
                 i % 2 == 0 ? 1000.0f : 1000000.0f, 0.1f);
    prev = curr;
  }
}
      ...
      for (int y = 0; y < H; y++)
        grid[x][y] =
            new Rigid(solver, {1, 1}, 1.0f, 0.5f,
                      {(float)x, (float)y + H * i * 2.0f + 5.0f, 0.0f});
...
static void sceneJointGrid(Solver *solver) {
  ...
  for (int x = 0; x < W; x++)
    for (int y = 0; y < H; y++)
      grid[x][y] = new Rigid(solver, {1, 1},
                             y == H - 1 && (x == 0 || x == W - 1) ? 0.0f : 1.0f,
                             0.5f, {(float)x, (float)y, 0.0f});
...
static void sceneNet(Solver *solver) {
  ...
  for (int x = 0; x < N / 4; x++)
    for (int y = 0; y < N / 8; y++)
      new Rigid(solver, {1, 1}, 1.0f, 0.5f,
                {(float)x - N / 8.0f, (float)y + 15.0f, 0.0f});
}
...
static void sceneFracture(Solver *solver) {
  ...
  Rigid *prev = 0;
  for (int i = 0; i <= N; i++) {
    Rigid *curr = new Rigid(solver, {1, 0.5f}, 1.0f, 0.5f,
                            {(float)i - N / 2.0f, 6.0f, 0.0f});
    if (prev)
      new Joint(solver, prev, curr, {0.5f, 0}, {-0.5f, 0},
                {INFINITY, INFINITY, INFINITY}, 500.0f);
    prev = curr;
  }
...
static void sceneCards(Solver *solver) {
  ...
  while (Nb) {
    ...
    for (int i = 0; i < Nb; i++) {
      if (i != Nb - 1) {
        new Rigid(solver, {cardThickness, cardHeight}, 1.0f, 0.7f,
                  {z + 0.25f, y + cardHeight * 0.5f - 0.02f, angle2});
      }
...
static void (*scenes[])(Solver *) = {
    sceneEmpty,          sceneGround,       sceneDynamicFriction,
    sceneStaticFriction, scenePyramid,      sceneCards,
    sceneRope,           sceneHeavyRope,    sceneHangingRope,
    sceneSpring,         sceneSpringsRatio, sceneStack,
    sceneStackRatio,     sceneRod,          sceneSoftBody,
    sceneJointGrid,      sceneNet,          sceneMotor,
    sceneFracture};

static const char *sceneNames[] = {"Empty",
                                   "Ground",
                                   "Dynamic Friction",
                                   "Static Friction",
                                   "Pyramid",

...
bool Manifold::initialize() {
  ...
  for (int i = 0; i < numContacts; i++) {
    ...
    vector2 rAW = rotate(bodyA->position.z(), contacts[i].rA);
    vector2 rBW = rotate(bodyB->position.z(), contacts[i].rB);

    // Precompute the constraint and derivatives at C(x-), since we use a
    // truncated Taylor series for contacts (Sec 4). Note that we discard the
    // second order term, since it is insignificant for contacts
    ...
    contacts[i].C0 = basis * (bodyA->position.head<2>() + rAW -
                              bodyB->position.head<2>() - rBW) +
                     vector2{Solver::COLLISION_MARGIN, 0};
  }
...
void Manifold::computeConstraint(float alpha) {
  for (int i = 0; i < numContacts; i++) {
    // Compute the Taylor series approximation of the constraint function C(x)
    // (Sec 4)
    float3 dpA = bodyA->position - bodyA->initial;
    float3 dpB = bodyB->position - bodyB->initial;

    C[i * 2 + 0] = contacts[i].C0.x() * (1 - alpha) + contacts[i].JAn.dot(dpA) +
                   contacts[i].JBn.dot(dpB);
    C[i * 2 + 1] = contacts[i].C0.y() * (1 - alpha) + contacts[i].JAt.dot(dpA) +
                   contacts[i].JBt.dot(dpB);
    ...
    // Check if the contact is sticking, so that on the next frame we can use
    // the old contact points for better static friction handling
    contacts[i].stick = abs(lambda[i * 2 + 1]) < frictionBound &&
                        abs(contacts[i].C0.y()) < Solver::STICK_THRESH;
  }
}

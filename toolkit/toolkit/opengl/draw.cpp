...
static const char consolines_nums[94] = {
    2,  6, 4,  10, 15, 16, 4,  5,  5,  3, 2,  3,  1,  1,  1,  13, 3,  6, 11,
    4,  8, 13, 2,  14, 15, 2,  4,  2,  2, 2,  7,  28, 3,  12, 10, 9,  4, 3,
    11, 3, 3,  5,  4,  2,  4,  3,  12, 7, 15, 8,  12, 2,  6,  2,  4,  2, 3,
    3,  3, 1,  3,  2,  1,  1,  11, 10, 8, 10, 12, 5,  26, 6,  4,  6,  4, 3,
    11, 6, 10, 9,  9,  5,  10, 5,  6,  2, 4,  2,  4,  3,  10, 1,  10, 7};
...
void draw_text3d(std::string text, math::vector3 location, math::quat rotation,
  ...
  std::vector<std::pair<math::vector3, math::vector3>> start_ends;
    ...
    if (text[i] == '\n') {
      ...
      for (int li = 0; li < lnum; li++) {
        ...
        math::vector3 start = math::vector3(
            ...
            yOffset - scale * height * (1.0f - ((float)yStartInt) / 128), 0.0f);

        start = rotation * start + location;

        // Compute the line end position in 3D space
        math::vector3 end = math::vector3(
            xOffset + scale * width * (((float)xEndInt) / 128),
            yOffset - scale * height * (1.0f - ((float)yEndInt) / 128), 0.0f);

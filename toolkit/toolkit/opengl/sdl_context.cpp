...
void sdl_context::init(unsigned int width, unsigned int height,
                       const char *title, int majorVersion, int minorVersion) {
  wnd_width = width;
  wnd_height = height;
...
void sdl_context::run(std::function<void(void)> mainLoop) {
  bool running = true;
  // Keep the same semantics: while window not closed
  while (running) {
    // poll will update internal states and handle SDL_QUIT internally
    poll();

    // detect quit: if window was destroyed or SDL quit event set a flag if
    // needed We'll consider window == nullptr as exit condition.
    if (!window)
      break;
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_KEYUP: {
      ...
      bool curState = (event.type == SDL_KEYDOWN) &&
                      (event.key.repeat == 0 ||
                       event.key.repeat == 1); // treat keydown as pressed
...
void sdl_context::begin_imgui() {
  ImGui_ImplOpenGL3_NewFrame();
  ImGui_ImplSDL2_NewFrame();
  ImGui::NewFrame();
}

void sdl_context::end_imgui() {
  ImGui::EndFrame();
  ImGui::Render();
  ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData());
}

bool sdl_context::is_key_pressed(int key) const {
  auto it = key_states.find(key);
  return it != key_states.end() && it->second;
}
bool sdl_context::is_key_triggered(int key) const {
  return triggered_keys.count(key) != 0;
}
bool sdl_context::is_key_untriggered(int key) const {
  return untriggered_keys.count(key) != 0;
}
bool sdl_context::is_mouse_button_triggered(int key) const {
  return triggered_mouse_keys.count(key) != 0;
}
bool sdl_context::is_mouse_button_untriggered(int key) const {
...
std::vector<SDL_GameController *> sdl_context::get_game_controllers() {
...
sdl_context::get_game_controller_analog_inputs(SDL_GameController *controller) {
  float tl, tr;
...
void sdl_context::set_vsync_state(bool enable) {
  should_vsync = enable;
  if (should_vsync)

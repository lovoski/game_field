...
std::vector<Plane> build_boundary_planes(convex_hull_collider *convex_hull,
                                         int target_face_idx);
...
void convex_convex_contact_manifold(convex_hull_collider *c1,
  ...
  if ((edge_normal_dot > chosen_normal1_dot + EPSILON) &&
    ...
    // face
    bool is_face1_the_reference_face = chosen_normal1_dot > chosen_normal1_dot;
    ...
    std::vector<Plane> reference_planes;
...
bool is_point_in_plane(const Plane &plane, vector3 position) {
...
bool plane_edge_intersection(const Plane &plane, const vector3 &start,

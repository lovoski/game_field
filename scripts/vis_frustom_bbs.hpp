      ...
      for (int i = 0; i < num_cascades; i++) {
        csm_split_depth[i + 1] =
            ...
            (1.0f - csm_split_lambda) *
                (ent_cam->z_near +
                 (i + 1) * (ent_cam->z_far - ent_cam->z_near) / num_cascades);
        auto [bbs_center, bbs_radius] = toolkit::frustom_bounding_sphere(
            csm_split_depth[i], csm_split_depth[i + 1], ent_cam->fovy_degree,
            scn_w, scn_h);
        toolkit::math::vector4 tmp_point;
        tmp_point << bbs_center, 1.0f;
        tmp_point = ent_trans.matrix() * tmp_point;
        toolkit::opengl::draw_wire_sphere(tmp_point.head<3>(), cam_comp.vp,
                                          bbs_radius);
        draw_view_frustom(ent_trans, ent_cam->fovy_degree, csm_split_depth[i],
                          csm_split_depth[i + 1], scn_w / scn_h, cam_comp.vp,
                          toolkit::opengl::Red);
      }
    }
  }
